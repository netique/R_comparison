...
server <- function(input, output, session) {
  ...
  inc_deaths <- reactive({
    vroom("https://onemocneni-aktualne.mzcr.cz/api/v2/covid-19/nakazeni-vyleceni-umrti-testy.csv") %>%
      select(
        date = datum,
        inc = prirustkovy_pocet_nakazenych,
        deaths = prirustkovy_pocet_umrti
      ) %>%
  ...
  hosp <- reactive({
   hosp <- vroom("https://onemocneni-aktualne.mzcr.cz/api/v2/covid-19/hospitalizace.csv")
  ...
  merged <- reactive({
    max_date <- Reduce(intersect, list(inc_deaths()$date, hosp()$date)) %>%
      max(na.rm = TRUE) %>%
      as_date()
  ...
  plot_data <- reactive({
    bind_rows(
      r_inc_data(), r_hosp_data(), r_asymp_data(), r_mild_data(), r_mod_data(),
  ...
  source("server/age_risk.R", local = TRUE, encoding = "UTF-8")
